...
This project uses [uv](https://docs.astral.sh/uv/) instead of Poetry.

| Task | Command |
|---|---|
| Install all deps (incl. dev) | `uv sync --group dev` |
| Install prod deps only | `uv sync --no-dev` |
| Add a runtime dependency | `uv add <package>` |
| Add a dev dependency | `uv add --group dev <package>` |
| Remove a dependency | `uv remove <package>` |
| Run a script/tool | `uv run <command>` |
| Update lockfile | `uv lock` |
| Run tests | `uv run pytest` |
| Run formatter | `uv run ruff check --select I --fix .` |

> **Never** use `pip install` directly — all packages must go through `uv` so `uv.lock` stays in sync.
